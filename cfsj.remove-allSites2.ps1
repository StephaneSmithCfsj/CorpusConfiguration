...
for ($index=$tmpRootColl.Count-1 ; $index-ge 0 ; $index–-)
{
  Write-Host "Removing site Content Database :" $tmpRootColl.Item($index).Name
  Remove-SPContentDatabase -Identity $tmpRootColl.Item($index).name -Confirm:$false -force
}
